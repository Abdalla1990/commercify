    ...
    render(){
      ...
      <div>
        <div className="div-container frames-ds-wrapper core-element">
          <dir className="c-frames-title"><h1>{title}</h1></dir>
          <div className="c-frame_images-container">{ images.map((image, index) => {
            return (index !== 1 ?
            <img className={`c-frames_image image-${index}`} src={image} />
            :
            <div className="c-frames_image" ><p className="content"  >{ content }</p></div>)
          })} </div>
        </div>
        <div className="div-container frames-mb-wrapper">
          <div className="c-frames-title"><h1>{title}</h1></div>
          <div className="c-frames-mb_image" > <p className="content"  > { content } </p></div>
          <div className="c-frames-mb_images-container">{ imagesMob.map(image => <img className="c-frames-mb_image" src={image} />)}</div>
        </div>
      </div>
			)
    }
